...
- **落代码**：核心编排与工具层由我实现（LangGraph + 3 个 MCP 子进程）。
...
- 缺陷防回归：地图 POI 误定位、多轮省略问法、参数空值等已修复并固化为例。
...
*本仓库为作品展示用途，保留产品方案摘要（`docs/产品方案摘要.html`）供对照阅读。面试相关准备材料（题库、简历、复盘）不在此仓库。*
